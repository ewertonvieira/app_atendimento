                                ...
                                <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                                    ...
                                    <form action="{{ route('atendimentos.update', $atendimento->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="text" id="status" value="{{ ucfirst($atendimento->status) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-100" readonly>
                                        </div>

                                        {{-- Descrição --}}
                                        <div class="mb-4">
                                            <label for="descricao" class="block text-sm font-medium text-gray-700">Descrição</label>
                                            <input type="text" name="descricao" id="descricao" value="{{ $atendimento->descricao }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="file" name="foto" id="foto{{ $atendimento->id }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" onchange="previewImage(event, 'preview{{ $atendimento->id }}')">
                                            @if ($atendimento->foto)
                                        ...
                                        <div class="mt-4 flex justify-end">
                                            <button type="button" onclick="closeModal('editModal{{ $atendimento->id }}')" class="px-4 py-2 bg-gray-500 text-white rounded-md shadow-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2">
                                                Cancelar
                                            </button>
                                            <button type="submit" class="ml-2 px-4 py-2 bg-indigo-500 text-white rounded-md shadow-md hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2">
                                                Salvar
                                            </button>
                                        </div>
                                    </form>
